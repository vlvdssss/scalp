"""Writer script – generates main_window.py. Delete after use."""
import pathlib
...
class MainWindow(QMainWindow):
    ...
    def _build_central(self) -> None:
        ...
        # Metric strip
        strip_outer = QWidget()
        strip_outer.setMaximumHeight(84)
        sr = QHBoxLayout(strip_outer)
        sr.setContentsMargins(14, 8, 14, 8)
        sr.setSpacing(8)

        def _card(label: str, accent: bool = False) -> QLabel:
            card = QFrame()
            ...
            vl.setSpacing(2)
            lk = QLabel(label)
            lk.setObjectName("metricLabel")
            lk.setAlignment(Qt.AlignmentFlag.AlignCenter)
            lv = QLabel("\u2014")
            lv.setObjectName("valueLabel")
        ...
        self._lbl_bid     = _card("BID")
        self._lbl_ask     = _card("ASK")
        self._lbl_spread  = _card("SPREAD")
        self._lbl_state   = _card("STATE", accent=True)
        self._lbl_bal     = _card("BALANCE")
        self._lbl_equity  = _card("EQUITY")
        self._lbl_dpnl    = _card("SESSION P&&L", accent=True)
        self._lbl_dtrades = _card("TRADES")
        self._lbl_conn    = _card("MT5")
...
pathlib.Path(r'C:\Scalper\app\src\ui\main_window.py').write_text(CONTENT, encoding='utf-8')
print("OK")
